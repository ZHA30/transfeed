...
运行时配置 schema 维护在 `src/config/schema.ts`，state 提交流程会自动生成 `config/feeds.schema.json` 到 state 目录，供编辑器和人工维护配置时使用。
...
- `translate.targetLanguage` 必填，例如 `zh-CN`、`zh-TW`、`ja`、`ko`。
- `translate.mode` 可选，允许 `translation` 或 `bilingual`；未配置时默认为 `bilingual`。
- `limit` 可选，必须是正整数，表示该 feed 的输入和输出窗口大小；未配置时使用代码默认值 `25`。
- `translate.fields` 必填，非空数组，只允许标准化后的 item 字段。
...
- `feedId`：状态文件使用的稳定 hash，基于 `path + url + targetLanguage + mode + fields + systemPrompt`。
...
- `bilingual` 模式下有译文和原文：输出 `译文¶原文`。
- `translation` 模式下有译文：输出 `译文`。
- 只有原文：输出原文。
- 只有译文：输出译文。
- 翻译失败：输出原文。
...
- `bilingual` 模式下，每个成功翻译的正文块，在原块内容后追加 `<span class="translated"><br>译文</span>`。
- `translation` 模式下，每个成功翻译的正文块替换为译文文本，保留外层块元素。
- `span` 由程序生成。
